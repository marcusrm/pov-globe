...
void loop() {
  ...
  //lightUP(strip.Color(50, 255, 255), 50);
...
void lightUpCol(){
 for(uint32_t i=0; i<strip.numPixels(); i++) {   
   uint8_t r = pgm_read_byte(&(Image[colIndex][3*i])); 
   uint8_t g = pgm_read_byte(&(Image[colIndex][3*i+1])); 
   uint8_t b = pgm_read_byte(&(Image[colIndex][3*i+2])); 
//   
//   uint8_t r = 255;
//   uint8_t g = 255;
//   uint8_t b = 255;
